...
// Rating function
function br_rating_function( $rating = 5 ) {
	$html = '';
	for( $i = 0; $i < $rating; $i++ ) {
...
// Tags function
function br_get_tax_function( $taxonomy = 'br-tag' ) {
...
// Tags function
function br_get_taxonomies( $taxonomy = 'listing_category', $hide_empty = false ) {
	$args = [
		'taxonomy' => $taxonomy,
...
function br_meta( $id = '', $value_type = true ){
    
    $value = get_post_meta( get_the_ID(), '_br_'.$id, $value_type );
    
    return $value;
}

// br
function br_get_styles_from_meta( $footer_bg_opt ) {
	$bg_styles = '';
...
// Blog Date Permalink
function br_blog_date_permalink(){
...
// Blog Excerpt Length
if ( ! function_exists( 'br_excerpt_length' ) ) {
	function br_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'br_posted_comments' ) ) :
    function br_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','br');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','br');
            } else {
                $comments = esc_html__( '1 Comment','br' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'br' );
        }
...
function br_iframe_match(){   
    $audio_content = br_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function br_embedded_media( $type = array() ){
...
// WP post link pages
function br_link_pages(){
    
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'br' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'br' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// Blog social sharing buttons
function br_social_sharing_buttons( $ulClass = '', $tagLine = '', $show_label = false ) {
...
if( ! function_exists('br_blog_single_post_navigation') ) {
	function br_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'br_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'br' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'br' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'br_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Br Comment Template Callback
function br_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'br' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'br'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'br' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'br' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'br_replace_reply_link_class');
function br_replace_reply_link_class( $class ) {
...
// theme logo
function br_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'br_site_logo_83x38' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( br_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( BR_DIR_IMGS_URI . 'logo.png' ).'" alt="'.esc_attr( get_bloginfo('name') ).'"></a>';
	}
...
function br_pull_right( $id = '', $condation ){
...
function br_get_textareahtml_output( $content ) {
...
function br_get_slider_shortcode_options( $field ) {
...
// html Style tag
function br_inline_bg_img( $bgUrl ){
    $bg = '';
...
function br_sidebar_opt(){

    $sidebarOpt = br_opt( 'br-blog-sidebar-settings' );
    $sidebar = '1';
...
function br_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'br' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'br' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function br_themify_icon(){
    return [
        'flaticon-shave' 	=> __('Razor Icon', 'br'),
        'flaticon-barber' 	=> __('Trimmer Icon', 'br'),
        'flaticon-null' 	=> __('Scissor Icon', 'br'),
    ];
}

// CTA section
function br_get_cta_area() {
	$cta_section_title      = !empty( br_opt( 'br_cta_section_title') ) ? br_opt( 'br_cta_section_title') : '';
	$cta_section_text       = !empty( br_opt( 'br_cta_section_text') ) ? br_opt( 'br_cta_section_text') : '';
	$cta_section_btn_label  = !empty( br_opt( 'br_cta_section_btn_label') ) ? br_opt( 'br_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( br_opt( 'br_cta_section_btn_url') ) ? br_opt( 'br_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function br_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = br_opt('br_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = BR_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = BR_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = BR_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
...
===========================================================*/
function br_latest_blog( $pNumber = 3, $post_order = 'DESC' ){
			...
			<div class="single_news">
				<div class="thumb">
					<a href="<?php the_permalink(); ?>">
						<?php
							if( has_post_thumbnail() ){
								the_post_thumbnail( 'br_home_blog_thumb_555x369', ['alt' => get_the_title() ] );
							}
						?>
					</a>
				</div>
				<div class="news_info d-flex">
					...
					<div class="news_meta">
						<span><?php echo br_featured_post_cat(); ?></span>
