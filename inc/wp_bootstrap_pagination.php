...
function br_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'br' ),
        'next_string'     => esc_html__( '&raquo;', 'br' ),
        'before_output'   => '<div class="br-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'br_pagination_defaults', $defaults )
    );
